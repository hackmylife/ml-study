...
# =============================================================================
def conv2d_sinple(x, W, b=None, stride=1, pad=0):
...
class Pooling2DGrad(Function):
    ...
    def forward(self, gy):
        ...
        indexes = (self.indexes.revel()
                   + xp.arange(0, self.indexes.size * KH * KW, KH, KW))

        gcol[indexes] = gy.reval()
...
def pooling(x, kernerl_size, stride=1, pad=0):
    return Pooling(kernerl_size, stride, pad)(x)
...
class Im2col(Function):
    ...
    def forward(self, x):
        ...
        y = im2col_array(x, self.kernel_size, self.stride,
